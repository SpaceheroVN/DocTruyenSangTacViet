...
async function taitruocchuongsau() {
    ...
    try {
        ...
        const themtext = (txt) => {
            ...
            if (txt.length > 0) {
                const chunks = chiadoanvanban(txt, maydoc === 'fpt' ? 2000 : 3000);                cacdoantext.push(...chunks);
            }
        };
        if (doctentruyen) {
...
async function phatdoanamthanh(boquanghi = false) {
    ...
    if (!doan.url) {
        try {
            ...
            document.body.style.cursor = 'default';
            if (idht === idluotphat) {
                if (err.message === 'Lỗi Azure' || (err.message && err.message.includes('403'))) {
                    duptrongweb('Hết quota Azure tháng này');
                    return;
                }
                solanloilientuc++;
...
function chuanbingam() {
    ...
    if (congcu === 'web') {
        ...
        cacdoanamthanh = taocacdoanmaydoc(cacnutdoan, congcu === 'fpt' ? 2000 : 3000).map(c => ({ ...c, url: null }));        if (doandaluu > 0) {
